...
const STEPS = [
  {
    num: "01",
    title: "Discover",
    desc: "Agent queries live listings — every entry is ChainLens-reviewed before going public.",
  },
  {
    num: "02",
    title: "Inspect",
    desc: "Before spending, the agent reads schemas, examples, latency, and recent failures.",
  },
  {
    num: "03",
    title: "Sign",
    desc: "Buyer signs a USDC ReceiveWithAuthorization for the chosen listing.",
  },
  {
    num: "04",
    title: "Execute",
    desc: "Gateway calls the seller API and applies policy + schema checks.",
  },
  {
    num: "05",
    title: "Settle",
    desc: "Success settles on ChainLensMarket. Failure drops the auth so no USDC moves.",
  },
];
      ...
      <div className={sStyles.inner}>
        <FadeIn>
          ...
          <p className={sStyles.sub}>
            ChainLens connects listing discovery, pre-call inspection, and Base settlement through a
            transparent x402 buyer flow.
          </p>
        </FadeIn>
        ...
        <FadeIn delay={400}>
          <div style={{ marginTop: "3rem" }}>
            <TerminalWindow title="agent.js — With ChainLens">
              ...
              <TLine />
              <TLine>
                <T.kw>const</T.kw> <T.val>listings</T.val> <T.out>{"= await fetch("}</T.out>
                <T.str>
                  &apos;https://chainlens.pelicanlab.dev/api/market/listings?q=weather&apos;
                </T.str>
                <T.out>{");"}</T.out>
              </TLine>
              <TLine>
              ...
              <TLine />
              <TLine>
                <T.kw>const</T.kw> <T.val>detail</T.val> <T.out>{"= await fetch("}</T.out>
              ...
              <TLine />
              <TLine>
                <T.kw>await</T.kw> <T.cmd>fetch</T.cmd>
                <T.out>(</T.out>
              ...
              <TLine />
              <TLine>
                <T.ok>✓ Seller response delivered · Settlement tx returned</T.ok> <Cursor />
              </TLine>
            </TerminalWindow>
          </div>
        </FadeIn>
