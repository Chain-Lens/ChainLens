...
export interface McpConfig {
  ...
  rpcUrl: string;
  /**
   * Private key for the buyer wallet used by `chainlens.request`.
   * Optional: discover + status work read-only, but request tools must have a signer.
   */
  walletPrivateKey?: `0x${string}`;
  /** Polling interval in ms for `chainlens.request` evidence polling. */
  pollIntervalMs: number;
  /** How long `chainlens.request` waits for a COMPLETED/REFUNDED state before giving up. */
  pollTimeoutMs: number;
}

export function loadMcpConfig(env: NodeJS.ProcessEnv = process.env): McpConfig {
  const apiBaseUrl = (env.CHAINLENS_API_URL ?? "http://localhost:3001/api").replace(/\/+$/, "");
  ...
  const pollIntervalMs = Number(env.CHAINLENS_POLL_INTERVAL_MS ?? "2000");
  const pollTimeoutMs = Number(env.CHAINLENS_POLL_TIMEOUT_MS ?? "120000");
