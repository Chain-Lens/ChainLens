...
Use `http://localhost:3001/api` for `CHAIN_LENS_API_URL` only when you are
running the ChainLens backend locally, and set it explicitly in the MCP config.
...
claude mcp add chain-lens \
  ...
  -e CHAIN_LENS_CHAIN_ID=84532 \
  -e CHAIN_LENS_RPC_URL=https://sepolia.base.org \
...
> ⚠ **If you later add `CHAIN_LENS_WALLET_PRIVATE_KEY` (see [Wallet / signing](#wallet--signing)),
...
> env var like `"CHAIN_LENS_WALLET_PRIVATE_KEY": "${CHAIN_LENS_WALLET_PRIVATE_KEY}"`.
...
### Option B — `CHAIN_LENS_WALLET_PRIVATE_KEY` (legacy, testnet-only)
...
> ⚠ **If you set `CHAIN_LENS_WALLET_PRIVATE_KEY`, use a throwaway Base Sepolia wallet
> with only faucet funds.** Never paste a mainnet key or a key that controls
> anything you care about. Setting both `CHAIN_LENS_WALLET_PRIVATE_KEY` and
...
The MCP tool prints a stderr warning on startup whenever `CHAIN_LENS_WALLET_PRIVATE_KEY`
...
Paid (requires `@chain-lens/sign` daemon or testnet `CHAIN_LENS_WALLET_PRIVATE_KEY`):
