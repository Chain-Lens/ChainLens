...
describe("SellerRegistry", function () {
  async function deployFixture() {
    const [owner, gateway, seller, other, stranger] =
      await hre.viem.getWalletClients();
    const registry = await hre.viem.deployContract("SellerRegistry", [
  ...
  describe("Deployment", function () {
    ...
    it("emits GatewayUpdated(0, gateway) on deploy", async function () {
      const { registry, gateway, publicClient } = await loadFixture(
        deployFixture,
      );
    ...
    it("reverts on zero gateway", async function () {
      await expect(
        hre.viem.deployContract("SellerRegistry", [zeroAddress]),
  ...
  describe("setGateway", function () {
    it("owner can rotate gateway and emits GatewayUpdated", async function () {
      const { registry, gateway, other, publicClient } = await loadFixture(
        deployFixture,
      );
    ...
    it("reverts on zero gateway", async function () {
      const { registry } = await loadFixture(deployFixture);
      await expect(
        registry.write.setGateway([zeroAddress]),
      ).to.be.rejectedWith(/zero gateway/);
    });
    ...
    it("reverts OwnableUnauthorizedAccount for non-owner", async function () {
      ...
      await expect(
        asOther.write.setGateway([other.account.address]),
  ...
  describe("registerSeller", function () {
    it("stores seller and indexes capabilities", async function () {
      ...
      expect(getAddress(s.sellerAddress)).to.equal(
      ...
      expect(
        (await registry.read.getSellersByCapability([CAP_A])).map(getAddress),
      ).to.deep.equal([getAddress(seller.account.address)]);
      expect(
        (await registry.read.getSellersByCapability([CAP_B])).map(getAddress),
      ).to.deep.equal([getAddress(seller.account.address)]);
      expect(await registry.read.isRegistered([seller.account.address])).to.be
        .true;
      expect(await registry.read.isActive([seller.account.address])).to.be.true;
    });

    it("emits SellerRegistered", async function () {
      const { registry, gateway, seller, publicClient } = await loadFixture(
        deployFixture,
      );
      const asGateway = await as(registry, gateway);
      await asGateway.write.registerSeller([
    ...
    it("reverts on zero seller", async function () {
      ...
      const asGateway = await as(registry, gateway);
      await expect(
        asGateway.write.registerSeller([
          zeroAddress,
          NAME,
          [CAP_A],
          METADATA,
    ...
    it("indexes two sellers under the same capability", async function () {
      const { registry, gateway, seller, other } = await loadFixture(
        deployFixture,
      );
      const asGateway = await as(registry, gateway);
      await asGateway.write.registerSeller([
      ...
      const list = (
        await registry.read.getSellersByCapability([CAP_A])
  ...
  describe("updateMetadataURI", function () {
    it("updates URI and emits SellerUpdated", async function () {
      ...
      await asGateway.write.updateMetadataURI([
        seller.account.address,
        "ipfs://new",
      ]);
      const s = await registry.read.getSellerInfo([seller.account.address]);
    ...
    it("reverts when seller not registered", async function () {
      ...
      const asGateway = await as(registry, gateway);
      await expect(
        asGateway.write.updateMetadataURI([
          seller.account.address,
          "ipfs://x",
        ]),
  ...
  describe("deactivate", function () {
    ...
    it("seller can deactivate self", async function () {
      ...
      expect(await fx.registry.read.isActive([fx.seller.account.address])).to
        .be.false;
    });
    ...
    it("unauthorized caller reverts", async function () {
      ...
      await expect(
        asStranger.write.deactivate([fx.seller.account.address]),
    ...
    it("reverts when seller not registered", async function () {
      ...
      const asGateway = await as(registry, gateway);
      await expect(
        asGateway.write.deactivate([other.account.address]),
      ).to.be.rejectedWith(/not found/);
    });
    ...
    it("reverts when already inactive", async function () {
      ...
      await asGateway.write.deactivate([fx.seller.account.address]);
      await expect(
        asGateway.write.deactivate([fx.seller.account.address]),
      ).to.be.rejectedWith(/already inactive/);
    });
  });
  ...
  describe("recordJobResult / getReputation", function () {
    ...
    it("returns neutral 5000 bps when no jobs recorded", async function () {
      const fx = await registered();
      expect(
        await fx.registry.read.getReputation([fx.seller.account.address]),
      ).to.equal(5_000n);
    });

    it("success path increments completed + earnings and emits event", async function () {
      const fx = await registered();
      await fx.asGateway.write.recordJobResult([
        fx.seller.account.address,
        true,
        USDC(3n),
      ]);
      expect(
        await fx.registry.read.jobsCompleted([fx.seller.account.address]),
      ).to.equal(1n);
      expect(
        await fx.registry.read.jobsFailed([fx.seller.account.address]),
      ).to.equal(0n);
      expect(
        await fx.registry.read.totalEarnings([fx.seller.account.address]),
      ).to.equal(USDC(3n));
  ...
  describe("view helpers", function () {
    it("getSellerInfo reverts for unknown seller", async function () {
      const { registry, other } = await loadFixture(deployFixture);
      await expect(
        registry.read.getSellerInfo([other.account.address]),
      ).to.be.rejectedWith(/not found/);
    });
    ...
    it("isRegistered/isActive false for unknown", async function () {
      const { registry, other } = await loadFixture(deployFixture);
      expect(await registry.read.isRegistered([other.account.address])).to.be
        .false;
  ...
  describe("Ownable2Step", function () {
    ...
    it("new owner can rotate gateway; old owner cannot", async function () {
      const { registry, owner, other, stranger } = await loadFixture(
        deployFixture,
      );
      await registry.write.transferOwnership([other.account.address]);
      const asOther = await as(registry, other);
      ...
      await expect(
        asOldOwner.write.setGateway([owner.account.address]),
      ).to.be.rejectedWith(/OwnableUnauthorizedAccount/);
    });
  });
});
